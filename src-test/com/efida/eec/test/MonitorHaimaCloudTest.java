...
public class MonitorHaimaCloudTest extends Thread {
...
//			"\r\n-----END RSA PRIVATE KEY-----";
        ...
        String url = "http://localhost:8080/reportmonitor/tpblf0th98c/"+timestamp+"/"+nonce+"/"+signature;
        JSONObject obj = new JSONObject();
        obj.put("servers", servers);
//		Log.msg(servers.toString(4));
	...
	public void run()
	{
		...
		while(running){
			synchronized (reports) {
				try {
					reports.wait(10000+random.nextInt(3000));
			...
			try {
				if( servers.length() > 0 ){
					...
					Log.msg("Not found report need to submit.");
				}
			} catch (Exception e1) {
	...
	public static void main(String[] args) {
		...
		MonitorHaimaCloudTest monitor = null;
		try
		{
	        //启动日志管理器
			...
			//先ZK读取程序数据
			local = Zookeeper.getInstance("124.207.62.22", 9088);
			...
			if( stat != null ){
				...
				monitor = new MonitorHaimaCloudTest();
