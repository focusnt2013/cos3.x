	...
	return this.each(function(){
		...
		var methods = {
			...
			go_to: function(index, easing, webkit){
				...
				/*Pioneer 自定义翻页事件处理 2013-03-26*/
				if(settings.onPageIndexChange != undefined)
				{
				   eval(settings.onPageIndexChange($this,methods,cells));
				}
		...
		if($this.data('promptumenu')){
			...
			if(cells.pages > 1 && settings.pages == true){
				...
				var page_links = "";
				/*Pioneer 自定义页码处理 2013-03-26*/
				if(settings.paginationFormat != undefined)
				{
					eval(settings.paginationFormat(pageObject,cells));
					page_links = pageObject.page_links;
				}
				else
				{
					page_links = '<a class="active">Page 1</a>';
			...
			/*Pioneer 自定义构建处理 2013-03-25*/
			if(settings.onBuilding != undefined)
			{
			   eval(settings.onBuilding($this,methods,cells));
			}
