...
});Event.Keys={enter:13,up:38,down:40,left:37,right:39,esc:27,space:32,backspace:8,tab:9,"delete":46};Event.Keys=new Hash(Event.Keys);
Event.implement({
	stop:function(){
...
if(g&&t[l]){t[l].reverseCombinator=i(D);}t[++l]={combinator:D,tag:"*"};}var o=k.expressions[n][l];if(r){o.tag=r.replace(m,"");}else{if(C){o.id=C.replace(m,"");
...
}else{if(B){B=B.replace(m,"");u=(u||"").replace(m,"");var E,H;
	alert(A);
	switch(A)
	{
		case"^=":
		...
		case"|=":
			...
			break;
		case"=":
			E=function(I){return u==I;};
			break;
		case"*=":
			E=function(I){return I&&I.indexOf(u)>-1;};
			break;
		case"!=":
			E=function(I){return u!=I;};
			break;
	...
	if(u==""&&(/^[*$^]=$/).test(A)){E=function(){return false;};}if(!E){E=function(I){return I&&H.test(I);
...
return(a[b])?a[b](c,e,d||document):null;};})()});
	if(window.$412==null)
	{
		Window.implement("$412",function(a,b){
...
return a.call(this,g,f);};})(Slick.search,Slick.find,Slick.match);
	if(window.$1113==null)
	{
		Window.implement("$1113",
			function(a){
				var f=new Elements;
...
});a.$family=Function.from("fx:css:value");return a;},serve:function(c,b){
			if(typeOf(c)!="fx:css:value"){c=this.parse(c);}
			var a=[];
			c.each(function(d){
	...
	search:function(a){
		...
		Array.each(document.styleSheets,function(f,e){
			...
			Array.each(g,function(k,h){
				...
				var j=(k.selectorText)?k.selectorText.replace(/^\w+/,function(i){return i.toLowerCase();}):null;
