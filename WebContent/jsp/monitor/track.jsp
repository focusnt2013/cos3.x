                ...
                <div id='divTree'>
					...
					<div id="rMenu">
						<ul>
							...
							<li id='liServerFunc' class="divider"></li>
							<ww:if test='sysadmin'>
							<li id='menuServer01' onclick='configControl()'><i class='skit_fa_icon_war fa fa-cog' style='width:16px;'></i> 修改伺服器主控配置</li>
							...
							<li id='menuServer21' onclick='monitordatabase()'><i class='skit_fa_icon_blue fa fa-database fa-fw' style='width:16px;'></i> 配置数据库监控
							...
							<li id='menuServer39' onclick='ssh()'><i class='skit_fa_icon fa fa-terminal fa-fw' style='width:16px;'></i> 安全远程控制SSH</li>
							<li id='menuServer32' onclick='openZookeeperServer()'><i class='skit_fa_icon fa fa-institution fa-fw' style='width:16px;'></i> 打开Zookeeper管理器</li>
   			...
   			<div class="panel-heading"><span><i class='skit_fa_icon fa fa-server'></i> <span id='headTitle'>集群伺服器监控</span></span>
				<span class='dropdown' style='float: right;cursor:pointer;display:none' id='spanServerMgr'>
				  <a href='#'
					...
					<ww:if test='sysadmin'>
					<li onclick='configControl()'><a><i class='skit_fa_icon_war fa fa-cog' style='width:16px;'></i> 配置伺服器主控程序</a></li>
					...
					<li class='divider'></li>
					<li onclick='monitordatabase()'><a><i class='skit_fa_icon_blue fa fa-database fa-fw' style='width:16px;'></i> 配置数据库监控</a>
					<li class='divider'></li>
					</ww:if>
					<li onclick='openFiles()'><a><i class='skit_fa_icon fa fa-folder-open' style='width:16px;'></i> 打开文件管理器</a></li>
					<li onclick='ssh()' id='liSsh'><a><i class='skit_fa_icon_war fa fa-terminal' style='width:16px;'></i> 安全远程控制SSH</a></li>
					<li onclick='openZookeeperServer()'><a><i class='skit_fa_icon fa fa-institution fa-fw' style='width:16px;'></i> 打开Zookeeper管理器</a></li>
					...
					<li onclick='wrapperlog()'><a><i class='skit_fa_icon fa fa-file-word-o fa-fw' style='width:16px;'></i> 查看主控日志</a></li>
...
//########################################################################
	...
	function onRightClick(event, treeId, treeNode) {
		...
		if( treeNode.isParent)
		{
			...
			document.getElementById( 'liServerFunc' ).style.display = "none";
		...
		else
		{
			...
			document.getElementById( 'liDelServer' ).style.display = treeNode.id==-1?"none":"";
			document.getElementById( 'liServerFunc' ).style.display = treeNode.id==-1?"none":"";
			var liMenuServers = $("li[id^='menuServer']");
			for(var i = 0; i < liMenuServers.length; i+=1 )
			{
				var li = liMenuServers[i];
...
function open(node)
{
	if( node )
	{
		...
		if( node.ip ){
			document.getElementById( 'headTitle' ).innerHTML = "伺服器监控： "+node.name+" "+node.title; 
...
function ssh(){
	var node = getServer(); if( node == null ) return;
	openView("安全远程控制SSH: "+node.name, "rpc!open.action?id="+node.id);
}
